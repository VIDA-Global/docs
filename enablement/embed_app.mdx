...
## Required parameters
- `authToken` – one‑time authentication token used by the embed page to log the user in automatically. If omitted, the page logs the user out before rendering
- `email` – the email that must match the authenticated user. If it does not match, the user is logged out.
