...
Your agent can use your SIP registration via the [Call Transfer](/docs/functions/call-transfer) function to send calls out via your SIP registration so it can dial extensions and other internal destinations. You simply need to prepend `pbx:` or `ext:` to your transfer destination.

Full details on SIP Registration configuration [can be found here](/docs/sip/sip-registration).
...
You can add IPs via the [API](/api-reference/sip/list-sip-ip-whitelist) or through the [Settings -> Developer page](/app/inbox?filter=priority&settings=true&showOrgSettings=true&settingsTab=developers) in your Vida account.
...
You can add outbound routes via the [API](/docs/api-reference/sip/add-sip-outbound-route) or through the [Settings -> Developer page](/app/inbox?filter=priority&settings=true&showOrgSettings=true&settingsTab=developers).
