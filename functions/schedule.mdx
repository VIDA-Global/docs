...
Vida can perform an HTTP POST request when a scheduled event occurs. This is available via our [Zapier integration](/integrations/zapier) or by configuring your own [webhook via our API](/api-reference/webhooks/list-webhooks).

## Paywall your schedule

Your Vida AI phone agent can require payment before performing other actions. This is especially useful when combined with the Schedule action to facilitate paid consultations. Check out the [Request Payment](/functions/payments) function for more information.
