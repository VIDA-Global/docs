...
The language option in your Vida AI agent configuration allows you to select the language your agent should be listening for. This is crucial as Vida uses a Speech-to-Text (STT) engine to understand and process spoken language accurately. 
...
Please be aware that certain AI Text-to-Speech (TTS) voices are not available when Spanish is selected as the language. This means that while your agent can understand and process calls in Spanish, the range of voices it can use to respond may be limited compared to when English is selected.
